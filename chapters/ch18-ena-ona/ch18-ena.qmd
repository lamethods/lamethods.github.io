...
keywords: "epistemic network analysis, ordered network Analysis, quantitative ethnography, learning analytics"
self-contained: true
include-in-header: 
  - text: |
...
```{r, echo = F}
ena.plot <- function (enaset, title = "ENA Plot", dimension.labels = c("", 
  ...
  ...) 
{
  ...
  plot = ENAplot$new(enaset, title, dimension.labels, font.size, 
    font.color, font.family, scale.to = scale.to, ...)
...
p <- ena.plot(set.ena, title = "FirstGame mean plot")  |>
  ena.plot.network(network = first.game.mean, colors = c("red"))
p$plot
```
...
```
```{r, results='asis', echo = F}
p<-ena.plot(set.ena, title = "SecondGame mean plot")  |>
  ena.plot.network(network = second.game.mean, colors = c("blue"))
p$plot
```
...
```
```{r, results='asis', echo = F}
p<-ena.plot(set.ena, title = "Subtracted: `FirstGame` (red) - `SecondGame` (blue)")  |>
  ena.plot.network(network = subtracted.mean * 5, # Optional rescaling of the line weights
                   colors = c("red", "blue"))
p$plot
```
...
```{r, results='asis', echo = F}
p<-ena.plot(set.ena, title = " points (dots), mean point (square), and confidence interval (box)") |> 
          ena.plot.points(points = first.game.points, colors = c("red")) |> 
          ena.plot.group(point = first.game.points, colors =c("red"), 
                         confidence.interval = "box")
p$plot
...
```
```{r, results='asis', echo = F}
p<-ena.plot(set.ena, title = "FirstGame mean network and its points") |> 
          ena.plot.network(network = first.game.mean, colors = c("red")) |>
          ena.plot.points(points = first.game.points, colors = c("red")) |> 
          ena.plot.group(point = first.game.points, colors =c("red"), 
                         confidence.interval = "box") 
...
```
```{r, results='asis', echo = F}
p<-ena.plot(set.ena, title = "SecondGame mean network and its points") |> 
          ena.plot.network(network = second.game.mean, colors = c("blue")) |>
          ena.plot.points(points = second.game.points, colors = c("blue")) |> 
          ena.plot.group(point = second.game.points, colors =c("blue"), 
                         confidence.interval = "box")
...
```
```{r, results='asis', echo = F}
p<-ena.plot(set.ena, title = "Subtracted mean network: `FirstGame` (red) - `SecondGame` (blue)")  |>
          ena.plot.network(network = subtracted.mean * 5,
          colors = c("red", "blue")) |>
          ena.plot.points(points = first.game.points, colors = c("red")) |> 
          ena.plot.group(point = first.game.points, colors =c("red"), 
...
```
```{r, results='asis', echo = F}
p<-ena.plot(set.ena, title = "Individual network: `FirstGame`.steven z") |> 
          ena.plot.network(network = unit.A.line.weights, colors = c("red")) |>
          ena.plot.points(points = unit.A.point, colors = c("red"))
p$plot
```
...
```{r, results='asis', echo = F}
p <- ena.plot(set.ena, title = "Individual network: `SecondGame`.samuel o") |> 
          ena.plot.network(network = unit.B.line.weights, colors = c("blue")) |>
          ena.plot.points(points = unit.B.point, colors = c("blue"))
p$plot
```
...
```
```{r, results='asis', echo = F}
p <- ena.plot(set.ena, title = "Subtracted network: `FirstGame`.steven z (red) - `SecondGame`.samuel o (blue)")  |>
          ena.plot.network(network = (unit.A.line.weights - unit.B.line.weights) * 5,
          colors = c("red", "blue")) |>
          ena.plot.points(points = unit.A.point, colors = c("red")) |> 
          ena.plot.points(points = unit.B.point, colors = c("blue"))
...
viewover(accum.ena, 
     ...
     conversation.by = c("Condition", "GroupName", "ActivityNumber"), # consistent with in 4.3.3
     codes = c("Performance.Parameters", "Design.Reasoning"), # codes of choice
     window = 7, filename = "view1") # consistent with in 3.3.4
```
...
```{r, echo= F, results = 'asis'}
viewover(accum.ona, # the object stored our connection accumulation results in 4.3.6
     wh = c("SecondGame::samuel o"), # the unit we are interested in
     units.by = c("Condition", "UserName"), # consistent with in 4.3.1 
     conversation.by = c("Condition", "GroupName", "ActivityNumber"), # consistent with in 4.3.3
     codes = c("Performance.Parameters", "Design.Reasoning"), # codes of choice
     window = 7, filename="view2")
```
...
### Using ONA outputs in other analysis
