...
affiliations: 
  ...
  - id: "*"
    name: "Corresponding author: Keefe Murphy `keefe.murphy@mu.ie`"
crossref:
  fig-title: '**Figure**'
  fig-labels: arabic
  title-delim: "**.**"
...
It is important to note that there is no universally applicable definition of what constitutes a cluster [@Hennig2015;@Hennig2016]. Indeed, in the absence of external information in the form of existing "true" group labels, different clustering methods can reveal different things about the same data. There are many different ways to cluster the same data set, and different methods may yield solutions with different assignments, or even differ in the number of groups they identify. Consequently, we present several cluster analysis algorithms in this chapter; namely, $K$-Means [@MacQueen1967] in Section \ref{sec:kmtheory}, a generalisation thereof, $K$-Medoids [@Kaufman1990-pam], in Section \ref{sec:kmedoids}, and agglomerative hierarchical clustering [@Kaufman1990-agnes] in Section \ref{sec:hclust}. We apply each method in a comparative study in our tutorial using R [@R2023], with applications to a data set about the participants from discussion forum of a massive open online course (MOOC) for teachers. 
...
The remainder of this chapter proceeds as follows. In Section \ref{sec:review}, we review relevant literature in which dissimilarity-based clustering methods have been applied in the realm of educational research. In Section \ref{sec:methods}, we describe the theoretical underpinnings of each method in turn and discuss relevant practical guidelines which should be followed to secure satisfactory performance from each method throughout. Within Section \ref{sec:tutR}, we introduce the data set of our case study in Section \ref{sec:data} and give an overview of some required pre-processing steps in Section \ref{sec:process}, and then present a tutorial using R for each clustering method presented in this chapter in Section \ref{sec:apps}, with a specific focus on identifying the optimal clustering solution in Section \ref{sec:sil}. Finally, we conclude with a discussion and some recommendations for related further readings in Section \ref{sec:disc}, with a particular focus on some limitations of dissimilarity-based clustering which are addressed by other frameworks in the broader field of cluster analysis.
...
There are a number of commonly-used linkage criteria which we now describe. A non-exhaustive list of such linkages follows ---only those which we use in the later R tutorial in Section \ref{sec:tutR}--- in which we let $\mathcal{A}$ and $\mathcal{B}$ denote two sets of observations, $\lvert\cdot\rvert$ denote the cardinality of a set, and $d(a, b)$ denote the distance between observations in those corresponding sets according to the chosen distance measure. We note that ties for the maximum or minimum distances for complete linkage and single linkage, respectively, are broken at random.
...
```{r, echo = FALSE}
#| label: "tbl-data"
#| tbl-cap: "Preview of MOOC centralities data set."
gt::gt_preview(df) |> gt::fmt_number(columns=5:7, decimals=3)
```
\normalsize 
...
```{r, echo = FALSE}
#| label: "tbl-demo"
#| tbl-cap: "Preview of the selected additional categorical demographic variables associated with the MOOC centralities data set."
gt::gt_preview(demog)
```
\normalsize
...
We now show how each of the clustering methods described above can be implemented in R, using these data throughout and taking care to address all practical concerns previously raised. For each method ---$K$-Means in Section \ref{sec:kmapp}, $K$-Medoids in Section \ref{sec:pamapp}, and agglomerative hierarchical clustering in Section \ref{sec:hcapp}--- we show clustering results following the method-specific guidelines for choosing $K$. However, we conclude by comparing results across different methods using the average silhouette width criterion to further guide the choice of $K$ in Section \ref{sec:sil}. We refrain from providing an interpretation of the uncovered clusters until Section \ref{sec:optimal}, after the optimal clustering solution is identified.
...
As previously stated in Section \ref{sec:elbow}, the so-called "elbow method" consists of plotting the range of $K$ values on the x-axis against the corresponding obtained TWCSS values on the y-axis and looking for an kink in the resulting curve.
...
As per the $K$-Means tutorial in Section \ref{sec:kmapp}, we can produce an elbow plot by running the algorithm over a range of $K$ values and extracting the minimised within-cluster total distance achieved upon convergence for each value of $K$. We demonstrate this for the `dist_euclidean` input below. 
...
Performing agglomerative hierarchical clustering is straightforward now that the distance matrices have already been created for the purposes of running `pam()`. All that is required is to specify the distance matrix and an appropriate linkage criterion as the `method` when calling `hclust()`. We demonstrate this below for a subset of all possible distance measure and linkage criterion combinations among those described in Section \ref{sec:linkage}. Recall that for the Ward criterion, the underlying distance measure is usually assumed to be squared Euclidean and that `"ward.D2"` is the correct `method` to use when the Euclidean distances are not already squared. For `method="centroid"`, we manually square the Euclidean distances.
