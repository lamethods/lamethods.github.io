...
affiliations:
  ...
  - id: "*"
    name: "Corresponding author: Jelena Jovanovic, `jeljov@gmail.com`"
crossref:
  fig-title: '**Figure**'
  fig-labels: arabic
  title-delim: "**.**"
...
*Component*, *Event*, and *Action* values co-occur (@tbl-counts):
...
```{r, fig.cap="Distribution of action types across the course weeks", fig.width = 7, fig.height=5, out.width="4in"}
#| label: fig-1
...
```
@tbl-table1 shows the resulting comparison of the built models. According to
...
After the regression models for weeks 1 to 5 are built and evaluated, we combine and compare their performance measures, with the results reported in @tbl-table2.
```{r, eval = knitr::is_html_output()}
...
As shown in @tbl-table2, in this case, we do not have a clear situation as
it was with the classification task (@tbl-table1), since the three
