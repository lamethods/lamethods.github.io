...
affiliations:
  ...
  - id: "*"
    name: "Corresponding author: Leonie V.D.E. Vogelsmeier, `l.v.d.e.vogelsmeier@tilburguniversity.edu`"
crossref:
  fig-title: '**Figure**'
  fig-labels: arabic
  title-delim: "**.**"
...
command can be used to inspect each variables' range (@tbl-1):
```{r, eval = F}
describe(dataset)
```
...
```{r, echo = F}
#| label: tbl-1
#| tbl-cap: "Descriptive statistics of the dataset variables"
describe(dataset) |> gt::gt() |> gt::fmt_number(columns = c(3,4,6,7,11:13),decimals=2)
```
...
equation modeling (see Chapter XX). With the first argument `data`, you
