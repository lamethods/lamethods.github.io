---
title: "Chapter 12. A modern approach to transition analysis and process mining with Markov models: A tutorial with R" 
format: 
  pdf:
...
affiliations:
  ...
  - id: 2
    name: "INVEST Research Flagship Center, University of Turku"
  - id: 3
    name: "Department of Social Research, University of Turku"
  - id: 4
  ...
  - id: "*"
    name: "Corresponding author: Jouni Helske, `jouni.helske@iki.fi`"
crossref:
  fig-title: '**Figure**'
  fig-labels: arabic
  title-delim: "**.**"
...
<!-- Combining the properties of the MMM and the HMM leads to the mixture hidden Markov model (MHMM). We can use this model if we want to include both time-constant clusters and time-varying hidden states.--> Combining the ideas of both time-constant clusters and time-varying hidden states leads to the concept of mixture hidden Markov model (MHMM). Here we assume that the population of interest consists of a finite number of subpopulations, each with their own HMM with varying transition and emission probabilities. For example, we could expect to find underlying groups which behave differently when estimating the progression of ability through the sequence of test scores, such as those that consistently stay on a low-ability or high-ability track (stayers) and those that move between low and high ability (movers). In this case, we need two transition matrices: the stayers' transition matrix allows for no transitions while the movers' transition matrix allows for transitioning between low and high ability, as illustrated in @tbl-transmatMHMM. 
...
<!-- Hidden Markov models are not particularly well scalable either (expect in terms of number of sequences via straightforward parallelization), and mixture versions of both MM and HMM are notoriously difficult to estimate with large number of clusters as well. So I changed highly scalable to relatively scalable... (J) -->
...
<!-- Changed LIBRARIES to PACKAGES. Library refers to the directory where the package is stored. (J) -->
...
<!-- Added information on two-channel simulation: "In order to make the data more realistic, we simulated two-channel data but assume that we only have data on one of the channels---this way our models will not be ``too good''." 

We can also mention that the other channel was imagined to be achievement level, but I'm not sure if this is helpful or not... (S)
-->
...
<!-- **We are doing single-channel analysis here** -->
<!-- In the following example we demonstrate estimating a two-state HMM with user-defined starting values. The code to do so for a multi-channel analysis is notably similar to the single-channel case (not shown): we need only to supply the constituent channels in the form of a `list` and count the number of observed states in each channel in `n_symbols`.  We fit the model to the two-channel sequence data on collaboration roles and achievement. Furthermore, we define the starting values for the initial state probabilities and the transition matrix ourselves but simulate starting values for the emission matrices. -->
...
<!-- For line wrap of the following error: -->
```{r, echo = FALSE}
...
<!-- Here var was 7:55 but there is only missing data on the last column so I changed the end point to 54 -->
...
<!-- I changed the ending, I thought this would be easier to understand. (Satu) Here's the old text: "There are also evident missing values in the sequences. However, there are no internal gaps; here, the missing values are due to the sequences being of unequal lengths ("void" in `TraMineR` terminology)." -->
...
We don't need markov model for this, we can just compute the proportions of the first time points as `round(prop.table(table(seq_data_all[,1])),3)` (J)

Nevertheless, the `seqtrate()` function only computes the transition probabilities but does not compute the initial probabilities. If we would like to compute these, we may need to estimate a Markov model.--> 

<!-- Suggested editing text with the first sentence. (S) -->
...
<!-- Consider revising the model estimation and the following paragraph for finding more reliable estimates or mention that the estimates are not reliable. -->
