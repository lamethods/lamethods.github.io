...
plot.group_tna <- function(x,...) {return(tna:::plot.group_tna(x, ..., edge.label.cex =1.3, mar = c(4.75,4.75,4.75,4.75), vsize = 12))}

plot.group_tna_communities <- function(x,...) {return(tna:::plot.group_tna_communities(x, ..., edge.label.cex =1.3, mar = c(4.75,4.75,4.75,4.75), vsize = 12))}
```
...
pivot_wider(cents, names_from = "Group", 
  ...
  kbl(col.names = c(names(cents)[2],
                    rep(c("1. Soc. trig.", "2. Arg. trig.", 
                          "3. Fac. dis.", "4. Inf. trig."),3)),
      digits = 2,linesep = "", booktabs = T) %>%
  kable_minimal() %>%
...
#|  - "Dyad 3"
dyads1 <- cliques(tna_group[["Socially triggered"]], size = 2, threshold = 0.15)
plot(dyads1, vsize = 24, edge.label.cex = 2.5, ask = FALSE)
```
...
```{r, fig.width=10, fig.height=7, fig.dpi = 900}
#| label: fig-by-dist
...
par(mar=c(3,6,3,2))
seqdplot(sequ, group = lcp_clusters, ncol = 5, cex.legend = 2, cex.names = 2,
         cex.axis = 1.82, xtlab = 1:18, cex.lab = 2, cex.main = 2.4)
