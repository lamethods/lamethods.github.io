...
```{r fig.height=6, fig.width=12, fig.cap = "Histograms of all variables"}
#| label: fig-histall
...
```{r fig.height=5, fig.width=10, message = F, fig.cap= "Correlations between  `Final_Grade` and other variables"}
#| label: fig-corr
...
evaluation_data_lm <- bind_cols(test_data, 
...
```{r, message=FALSE, warning=FALSE, layout = c(1,1), fig.cap = "Linear regression plots", fig.width=5, fig.height=3, fig.dpi = 900}
...
We can plot the performance metrics to compare them across the models (@fig-comparing-models).
